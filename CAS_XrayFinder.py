...
def grid_search(i: int, n: int=2048, m: int=2040, search: bool=None, size='3x3') -> tuple:
    ...
    try: # Convert 2D Search Grid corners into 1D pixel coordinate values.
        ...
        if size == '3x3': # 3x3
            return (i, i-n-1, i-n, i-n+1, i-1, i+1, i+n-1, i+n, i+n+1)

        elif size == '5x5': # 5x5
            return (i, i-2*n-2, i-2*n-1, i-2*n, i-2*n+1, i-2*n+2, i-1*n-2, i-1*n-1, i-1*n, i-1*n+1, i-1*n+2, i-2, i-1, i+1, i+2, i+1*n-2, i+1*n-1, i+1*n, i+1*n+1, i+1*n+2, i+2*n-2, i+2*n-1, i+2*n, i+2*n+1, i+2*n+2)

        elif size == '7x7':
            return (i, i-3*n-3, i-3*n-2, i-3*n-1, i-3*n, i-3*n+1, i-3*n+2, i-3*n+3,
...
def xrf_analysis(filenames, queue_time, queue_data, event, parameters, is_tiff_file, roi, LR, scale, sg_size, cx, cy, r):
    """
    ...
    try:
        ...
        for file_count, file in enumerate(filenames):
            ...
            for i in coords_above_threshold:
                ...
                coords = np.array(grid_search(i, n=cols, m=rows, search = searchgrid, size = sg_size))
                ...
                if not singlepix:
                    ...
                    while not set(mp_searched) == set(mp_coords):
                        ...
                        c = np.array(grid_search(nw_i, n=cols, m=rows, search = searchgrid, size = sg_size))
                        if -10001 in arr[c]:
                            rm = 1
                            break
