...
class ParamsWindow:
    ...
    def __init__(self, win, nwin, params, b_quit):
        ...
        self.lbl_cx = tkr.Label(win, text="x - ROI Centre (pixels)")
        self.lbl_cy = tkr.Label(win, text="y - ROI Centre (pixels)")
        self.lbl_r = tkr.Label(win, text="Radius - ROI (pixels)")
        ...
        self.text_cx = tkr.Entry(win, bd=3, width = 20) # cx
        self.text_cy = tkr.Entry(win, bd=3, width = 20) # cy
        self.text_r = tkr.Entry(win, bd=3, width = 20) # r
        ...
        self.clicked.set( "All" ) # Default value is All
        ...
        self.clickedLR.set( "All" ) # Default value is All
        ...
        self.optionsSG = ["3x3", "5x5", "7x7"] 
        self.clickedSG = tkr.StringVar()
        self.clickedSG.set( "3x3" ) # Default value is 3x3
        self.dropSG = tkr.OptionMenu(win, self.clickedSG , *self.optionsSG)
        ...
        self.dropSG.grid(row=3, column=2, sticky="nsew", padx=5, pady=5)
        ...
        self.lbl_cx.grid(row=11, column=1, padx=5, pady=5)
        self.lbl_cy.grid(row=12, column=1, padx=5, pady=5)
        self.lbl_r.grid(row=13, column=1, padx=5, pady=5)
        
        self.text_cx.grid(row=11, column=2, padx=5, pady=5)
        self.text_cy.grid(row=12, column=2, padx=5, pady=5)
        self.text_r.grid(row=13, column=2, padx=5, pady=5)
        
        self.button_params.grid(row = 14, column = 1, padx=5, pady=5)
        self.button_defaults.grid(row = 15, column = 1, padx=5, pady=5)
        self.drop.grid(row=14, column=2, padx=5, pady=5)
        self.dropLR.grid(row=14, column=3, padx=5, pady=5)
    ...
    def update_display(self):
        ...
        self.text_cx.delete(0, tkr.END)
        self.text_cy.delete(0, tkr.END)
        self.text_r.delete(0, tkr.END)
        ...
        self.clickedSG.set(self.params.sg_size)
        
        self.text_cx.insert(0, self.params.cx)
        self.text_cy.insert(0, self.params.cy)
        self.text_r.insert(0, self.params.r)
    ...
    def update_params(self, win):
        ...
        self.params.sg_size = self.clickedSG.get()
        print(self.clickedSG.get())
        print(self.params.sg_size)
        

        cx = str(self.text_cx.get())
        if cx.isdigit():
            self.params.cx = int(cx)
        
        cy = str(self.text_cy.get())
        if cy.isdigit():
            self.params.cy = int(cy)
            
        r = str(self.text_r.get())
        if r.isdigit():
            self.params.r = int(r)
