...
class MyWindow:
    ...
    def __init__(self, win):
        ...
        self.button_create_df.grid(row = 1, column = 1,  padx=20, pady=2)   # Create Dark Frame
        self.button_create_hp.grid(row= 1, column = 2, padx=5, pady=2)      # Create Hot Pixel Map
        self.button_load_df.grid(row = 2, column = 1,  padx=20, pady=2)     # Select Dark Frame

        self.button_hist_df.grid(row = 17, column= 2,padx=20, pady=5)
        self.button_hist_sub.grid(row =17, column= 1,padx=20, pady=5)
        self.button_hist_save.grid(row=18, column=2, padx=20, pady=5)

        self.button_load_hp.grid(row = 2, column = 2, padx=20, pady=2)      # Select Hot Pixel Map
        self.button_params.grid(row = 3, column = 1, padx=5, pady=2)        # Update Parameters
        self.button_data.grid(row = 3, column = 2,  padx=5, pady=2)         # Sample Data File
        self.button_datas.grid(row = 7, column = 1,  padx=20, pady=2)       # Start XRF Analysis
        self.button_quit.grid(row = 14, column = 1,  padx=20, pady=2)       # Quit
        lbl1.grid(row = 4, column = 1, padx=5, pady=2)                      # Text labels
        lbl2.grid(row = 5, column = 1, padx=5, pady=2)
        lbl3.grid(row = 6, column = 1, padx=5, pady=2)
        self.text_thresh.grid( row = 4, column = 2, padx=5, pady=2)         # Entry boxes
        self.text_thresh2.grid(row = 5, column = 2, padx=5, pady=2)
        self.text_searchgrid.grid(row = 6, column = 2, padx=5, pady=2)
        self.my_progress = ttk.Progressbar(win, orient=tkr.HORIZONTAL,      # Progress Bar
                                           length=380, mode = 'determinate')
        self.my_progress.grid(row=10, column=1, columnspan=2, padx=20, pady=2)

        # Frame for time displays
        self.time_frame = tkr.Frame(win)
        self.time_frame.grid(row=9, column=1, columnspan=2, sticky="nsew", padx=20, pady=2)
        ...
        image_frame.grid(row = 1, column = 4, rowspan=20, padx=20, pady=2, sticky="nsew")
        ...
        # Switch Graph button
        button_calc_stop = tkr.Button(win, command = self.stop_calc, bg = 'mistyrose')
        ...
        self.console.grid(row=11, column=1, columnspan=2, rowspan=2, padx=20, pady=0, sticky="nsew")
