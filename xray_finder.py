...
def xrf_analysis(filenames, queue_time, queue_data, event, parameters, is_tiff_file, roi, LR, scale):
    ...
    try:
        ...
        for file_count, file in enumerate(filenames):
            ...
            if filter1 == filter2 == True: filter2 = False
            ...
            # n, bins, patches = plt.hist(arr, bins=np.arange(xmin, xmax+nr_bins, nr_bins), facecolor='green', alpha=0.75, density=True)
            # bin_max = bins[np.where(n == n.max())]
            # arr = arr - np.mean(arr)
            ...
            # const_offset = np.mean(pl_arr[subset])
            # const_offset = np.mean(pl_arr[subset][pl_arr[subset] < 4090])
            # print(const_offset)
            # print(pl_arr[0:10])
            print(np.mean(pl_arr))
            const_offset = 0
            threshold = threshold + const_offset
            sec_threshold = sec_threshold + const_offset
            # print(threshold)
            # print(sec_threshold)
